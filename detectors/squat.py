...
class SquatDetector(BaseExercise):
    ...
    def process(self , landmarks):
        left_knee_angle = self.calculate_angle(
            self.get(landmarks , self.LEFT_HIP),
            self.get(landmarks , self.LEFT_KNEE),
            self.get(landmarks , self.LEFT_ANKLE)
        )

        right_knee_angle = self.calculate_angle(
            self.get(landmarks , self.RIGHT_HIP),
            self.get(landmarks , self.RIGHT_KNEE),
            self.get(landmarks , self.RIGHT_ANKLE)
        )
